...
const ThemePicker = ({ onClose }) => {
  ...
  useEffect(() => {
    measureTabs();
    const timer = setTimeout(measureTabs, 150); // Re-measure after modal animates
  ...
  const getActiveTabStyle = () => {
    ...
    const base = {
      ...
      left: activeTabPos ? `${activeTabPos.offsetLeft}px` : `calc(${activeTabIdx} * (100% / ${N}))`,
            ...
            {tabs.map((t) => (
              <button
                ...
                style={{
                  ...
                  color: tab === t ? getActiveTabLabelColor() : theme.text2,
