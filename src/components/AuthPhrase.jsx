...
import { deriveZchatIdentityFromPhrase, deriveZchatIdentityFromSeed, deriveAesKeyFromPrivateKey } from "../utils/zchatIdentity";
import {
  encryptWithKey,
  decryptWithKey,
...
import {
  ...
  loadMasterKeyForUser,
  clearMasterKeyForUser,
  saveMasterKeyForUser,
...
const AuthPhrase = ({ onAuthSuccess }) => {
  ...
  // Sync remember me state when component becomes visible
  useEffect(() => {
    const handleVisibility = () => {
      if (document.visibilityState === "visible") {
        const enabled = isRememberMeEnabled();
        console.log("[Remember me] visibility check, enabled:", enabled);
  ...
  // ── Compatibility aliases for dead UI code (no-op, prevents crashes) ──
  const [pinEnabled, setPinEnabled] = useState(false);
  const rememberMe = rememberEnabled;
  const setRememberMe = setRememberEnabled;
  const [pinInput, setPinInput] = useState("");
  const [pinConfirm, setPinConfirm] = useState("");
  const [unlockPin, setUnlockPin] = useState("");
  const hasPinVault = hasPasswordVault;
  const setHasPinVault = setHasPasswordVault;
  const pinVaultEntries = vaultEntries;
  const setPinVaultEntries = setVaultEntries;
  const pinLockRemaining = 0;
  const setPinLockRemaining = () => {};
  // Dead utility function aliases (no-ops for cosmetic PIN UI code)
  const getPinGuardStatus = () => ({ locked: false, remainingMs: 0 });
  const registerPinFailure = () => ({ remainingMs: 0 });
  const registerPinSuccess = () => {};
  const formatCooldown = () => "";
  const isValidPin = () => false;
  const getPinStrengthError = () => "";
  const decryptPhraseWithPin = async () => { throw new Error("not available"); };
  const PIN_LENGTH = 6;
  const disableRememberMe = () => clearRememberedMasterKey();
  const loadPinVault = async () => null;
  const clearPinVault = async () => {};
  const savePinVault = async () => {};
  ...
  const persistMasterKeyLocally = async (userId, privateKey, options = {}) => {
    const uid = String(userId || "").trim().toLowerCase();
    const key = String(privateKey || "").trim().toLowerCase();
    if (!uid || !/^[0-9a-f]{64}$/i.test(key)) {
      throw new Error("Invalid master key for persistence");
    }
    await saveMasterKeyForUser(uid, key);
    if (options.remember) {
  ...
  const handleVerifyPhrase = async () => {
    ...
    try {
      const created = await signUpWithPhrase(phrase, rememberEnabled);
      ...
      // Store Ed25519 signing keys for request auth
      ...
      if (rememberEnabled) {
        await persistMasterKeyLocally(created.userId, created.privateKey, {
          remember: true,
        });
      } else {
        clearRememberedMasterKey();
  ...
  const handleAccountLogin = async (account, useBiometric = false, providedPin = null) => {
    try {
      if (!account || !account.userId) {
        throw new Error("Invalid account data");
      }

      const hasPin = pinVaultEntries.some(e => e.userId === account.userId);
      let savedMasterKey = null;

      if (hasPin) {
        if (!providedPin) {
          throw new Error("PIN required for this account");
        }
        const { loadPinVaultForUser } = await import("../utils/secureStorage");
        const { decryptMasterKeyWithPin } = await import("../utils/pinVault");
        const vault = await loadPinVaultForUser(account.userId);
        savedMasterKey = await decryptMasterKeyWithPin(vault, providedPin);
      } else {
        savedMasterKey = await loadMasterKeyForUser(account.userId);
        if (!savedMasterKey) {
      ...
      const signedIn = await signInWithPrivateKey(savedMasterKey, rememberEnabled);
      ...
      // Store Ed25519 signing keys for request auth
      const identity = deriveZchatIdentityFromSeed(savedMasterKey);
      setSigningKeys(identity.ed25519PrivateKeyHex, identity.ed25519PublicKeyHex);

      if (rememberEnabled) {
        await persistMasterKey(savedMasterKey, "");
      }
  ...
  const handleRemoveAccount = async (acc, e) => {
    ...
    try {
      ...
      await clearRememberedMasterKey();
      await removeAccount(acc.userId);
  ...
  const handleSelectAccountOption = async (option) => {
    ...
    if (option.hasSavedAccount && option.account) {
      ...
      try {
        await handleAccountLogin(option.account, false, unlockPin || null);
      } catch (e) {
  ...
  const handleBiometricAccountOption = async (option, e) => {
    ...
    try {
      ...
      setSelectedAccount(option.account);
      setAccountsLoading(true);
      await handleAccountLogin(option.account, true, null);
    } catch (err) {
  ...
  const handleRemoveAccountOption = async (option, e) => {
    ...
    try {
      if (option.hasSavedAccount) {
        ...
        await clearMasterKeyForUser(option.userId);
      } else {
  ...
  const handleSignIn = async (e, preferredMode = "auto") => {
    ...
    if (rememberEnabled && pinEnabled) {
      if (!pinInput || !pinConfirm || pinInput !== pinConfirm) {
        showMsg("Please set and confirm your PIN to enable Remember Me (Recommended).", "error");
        return;
      }
    }
    ...
    try {
      const resolvedPhrase = phraseInput.trim();
      ...
      // Try signin — if phrase not registered, will throw
      const signedIn = await signInWithPhrase(resolvedPhrase, rememberEnabled);
      ...
      // Store Ed25519 signing keys for request auth
      const identity = deriveZchatIdentityFromPhrase(resolvedPhrase);
      setSigningKeys(identity.ed25519PrivateKeyHex, identity.ed25519PublicKeyHex);

      if (rememberEnabled) {
        await persistMasterKeyLocally(signedIn.userId, signedIn.privateKey, {
          remember: true,
        });
      } else {
        clearRememberedMasterKey();
      ...
      // If PIN is enabled, encrypt master key with PIN and save to vault
      if (rememberEnabled && pinEnabled && pinInput) {
        const { encryptMasterKeyWithPin } = await import("../utils/pinVault");
        const encryptedVault = await encryptMasterKeyWithPin(signedIn.privateKey, pinInput);
        await savePinVaultForUser(signedIn.userId, encryptedVault, username || "Me");
      }
              ...
              <div style={{ marginTop: 12, marginBottom: 12 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    color: rememberMe && pinEnabled ? theme.success : theme.text3,
                    fontSize: 12,
                    fontWeight: 600,
                    cursor: "pointer",
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    background: "transparent",
                    color: rememberMe && !pinEnabled ? theme.warning : theme.text3,
                    fontSize: 12,
                    fontWeight: 600,
                    cursor: "pointer",
                    transition: "all 0.15s",
                ...
                <AnimatePresence initial={false}>
                  {pinEnabled && (
                    ...
                    >
                      ...
                      <div style={{ marginBottom: 12, opacity: (pinInput && pinConfirm && pinInput === pinConfirm) ? 1 : 0.4, transition: "opacity 0.2s" }}>
                        <FancyCheckbox
                          checked={biometricEnabled}
                          disabled={!(pinInput && pinConfirm && pinInput === pinConfirm)}
                          onToggle={async () => {
                            if (!biometricEnabled) {
                              try {
                                ...
                                showMsg("Biometric registered!", "success");
                              } catch (e) {
                  ...
                  >
                    ...
                    />
                      <button
                        ...
                        onClick={async () => {
                          if (!selectedAccount) {
                            showMsg("Please select an account first", "error");
                            return;
                          }
                          setAccountsLoading(true);
                          try {
                            await handleAccountLogin(selectedAccount, false, unlockPin);
                          } catch (err) {
