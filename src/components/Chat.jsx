...
function getMasterKeyFromStorage(fallback = "") {
  const stored = sessionStorage.getItem("userPrivateKey") || "";
  if (isValidMasterKey(stored)) return stored;
  if (isValidMasterKey(fallback)) return fallback;
  return "";
}
...
const Chat = ({
  ...
  // Sync editInput when editingMsg changes
  useEffect(() => {
    if (editingMsg) {
      const contentValue = editingMsg.content ?? "";
      console.log("[useEffect editInput] syncing with editingMsg, content:", contentValue);
      setEditInput(contentValue);
    }
  }, [editingMsg]);
  ...
  useEffect(() => {
    ...
    console.log("[Chat init] Starting with:", {
      conversationId: currentConvId,
      userId: user?.id,
      hasEcdhKey: !!ecdhPrivateKey,
      hasOtherUser: !!initialOtherUser,
      otherUserPublicKey: initialOtherUser?.publicKey ? "present" : "null",
    });

    const init = async () => {
      if (!user?.id || !currentConvId) {
        console.log("[Chat init] Bailing: missing user.id or currentConvId", { userId: user?.id, convId: currentConvId });
        if (!cancelled) {
          setStatus("");
      ...
      if (initialOtherUser) {
        ...
        // Load ratchet state from server FIRST (cross-device support)
        // Only create fresh state if no state exists on the server
        ...
        if (!ratchetState && ecdhPrivateKey) {
          try {
            ...
            // Fall through to createRatchetState
          }
        }
        ...
        console.log("[Chat init] Cache check:", {
          hasPreloaded: !!preloadedMessages,
          preloadedLen: preloadedMessages?.length,
          hasCached: !!cachedMessages,
          cachedLen: cachedMessages?.length,
        });

        // CRITICAL FIX: Preloaded messages from initUser() are raw encrypted objects
        // They have encrypted_content but NO content field. We must check if they're
        // actually decrypted before using them.
        const hasDecryptedContent = preloadedMessages && preloadedMessages.length > 0 &&
          preloadedMessages.some(m => m.content && m.content !== "");
        
        if (hasDecryptedContent) {
          // Preloaded messages already have decrypted content - use them
          serverLoadedRef.current = true;
          setMessages(preloadedMessages);
        } else if (preloadedMessages && preloadedMessages.length > 0) {
          // Preloaded messages are encrypted but not decrypted - need to decrypt them
          console.log("[Chat init] Preloaded messages are encrypted, decrypting via loadMessages...");
          shouldLoadFromDB = true;
          // Don't set serverLoadedRef yet - wait for decryption to complete
        ...
        // CRITICAL FIX: If preloaded messages are encrypted, loadMessages MUST run
        // to decrypt them before displaying
        if (shouldLoadFromDB) {
          // background sync initiated (log removed)
          // Background fast sync: do not show visual syncing badge
          console.log("[Chat init] Background loadMessages starting...");
          
          // CRITICAL: If preloaded messages are encrypted (no decrypted content),
          // we MUST wait for decryption to complete before showing the chat
          const needsDecryption = preloadedMessages && preloadedMessages.length > 0 && !hasDecryptedContent;
          
          if (needsDecryption) {
            // Wait for decryption - keep initialising state true
            console.log("[Chat init] Waiting for message decryption before displaying...");
            const fresh = await loadMessages(currentConvId, 60, false, true);
            console.log("[Chat init] Decryption complete:", fresh ? fresh.length : "null", "messages");
            ...
            // Messages already decrypted or no preloaded messages - run in background
            loadMessages(currentConvId, 60, false, true).then((fresh) => {
              console.log("[Chat init] Background loadMessages returned:", fresh ? fresh.length : "null", "messages");
              if (fresh && fresh.length > 0) {
                serverLoadedRef.current = true;
                setMessages(fresh);
        ...
        // Don't mark as seen here — only mark when messages are actually visible in viewport
      ...
      if (cached && !cancelled) {
        console.log("[Chat init] Fallback: using cached meta");
  ...
  useEffect(() => {
    const masterKey = getMasterKeyFromStorage(ecdhPrivateKey);
    if (!conversationId || !isValidMasterKey(masterKey)) return;
    let cancelled = false;

    // Only restore messages if server hasn't loaded yet
    const checkAndRestore = () => {
      if (serverLoadedRef.current) return;
      cacheGet(masterKey, convMsgCacheKey(user?.id, conversationId))
        .then((cached) => {
    ...
    if (unseenMsgIdsRef.current.size > 0) {
      ...
      for (const msgId of unseenMsgIdsRef.current) {
        ...
        if (isVisible) {
          ...
          newlyVisibleIds.push(msgId);
          removedAny = true;
        }
      }

      if (removedAny) {
        // Mark visible messages as seen on server
        if (newlyVisibleIds.length > 0 && conversationId) {
          markMessagesAsSeen(conversationId).catch(() => {});
        }
  ...
  useEffect(() => {
    ...
    ch
      ...
      .on(
        ...
        (p) => {
          ...
          if (isIncoming && p.new?.is_edited && p.new?.encrypted_content) {
            // Check if content actually changed (avoid unnecessary decrypts)
            setMessages((prev) => {
      ...
      // Message DELETE — remove deleted messages from UI and cache
      .on(
        "postgres_changes",
        {
          event: "DELETE",
        ...
        (p) => {
          if (!p.old?.id) return;
          const msgId = String(p.old.id);
          const masterKey = getMasterKeyFromStorage(ecdhPrivateKey);
          setMessages((prev) => {
            const updated = prev.filter((m) => String(m.id) !== msgId);
            _setConvMsgs(conversationId, updated);
            if (isValidMasterKey(masterKey)) {
              cacheSet(masterKey, convMsgCacheKey(user?.id, conversationId), updated).catch(() => {});
            }
            return updated;
          });
        },
        ...
        async (p) => {
          ...
          setReactions((prev) => {
            ...
            if (tmpIdx !== -1) {
              ...
              const masterKey = getMasterKeyFromStorage(ecdhPrivateKey);
              if (isValidMasterKey(masterKey)) {
                cacheSet(masterKey, convReactCacheKey(user?.id, conversationId), updated).catch(() => {});
              }
              return updated;
            }
      ...
      .on(
        ...
        async (p) => {
          ...
          const masterKey = getMasterKeyFromStorage(ecdhPrivateKey);
          if (isValidMasterKey(masterKey)) {
            cacheSet(masterKey, convReactCacheKey(user?.id, conversationId), map).catch(() => {});
          }
        },
      )
      // Reaction UPDATE
      ...
      .on(
        ...
        async (p) => {
          ...
          if (isValidMasterKey(masterKey)) {
            setTimeout(() => {
               cacheSet(masterKey, convMsgCacheKey(user?.id, conversationId), _getConvMsgs(conversationId) || []).catch(() => {});
            }, 0);
          }
        },
      )
        ...
        (async () => {
          try {
            ...
            for (const m of pendingToDecrypt) {
              ...
              const dec = await decryptWithRatchet(parsed.c, convKey, ecdhPrivateKey, Number(parsed.n), isOutgoing).catch(() => null);
              if (dec) {
                const existing = _getConvMsgs(conversationId) || [];
            ...
            if (changed) {
              ...
              const masterKey = getMasterKeyFromStorage(ecdhPrivateKey);
              if (isValidMasterKey(masterKey)) {
                await import("../utils/cache").then((c) => c.cacheSet(masterKey, convMsgCacheKey(user?.id, conversationId), final)).catch(() => {});
              }
            }
          } catch (e) {
            // ignore
  ...
  const loadMessages = async (convId, limit = 60, showSync = true, bypassCache = false) => {
    console.log("[loadMessages] Called with:", { convId, limit, showSync, bypassCache });
    if (!isLikelyUserId(convId)) {
      console.log("[loadMessages] convId is not a valid user ID, bailing out");
    ...
    const run = async () => {
      ...
      try {
        console.log("[loadMessages] Fetching from server...");
        const msgs = await getMessagesWithFriend(convId, limit, bypassCache);
        console.log("[loadMessages] Server returned:", msgs ? msgs.length : "null", "messages");
      if (!msgs) {
        setMessages([]);
        return;
      }
      ...
      for (const m of msgs) {
        ...
        if (
          ...
          !cached.content.startsWith("[Unable to decrypt") &&
          !cached.content.startsWith("[Invalid message") &&
          !cached.content.startsWith("[Encryption keys")
        ) {
          console.log("[loadMessages] Using cached plaintext for:", m.id);
          processed.push({
            ...m,
        ...
        const isIncoming = m.receiver_id === user?.id;
        console.log("[loadMessages] Processing message:", {
          id: m.id,
          isIncoming,
          sender: m.sender_id?.slice(0, 16) + "...",
          receiver: m.receiver_id?.slice(0, 16) + "...",
          hasEncryptedContent: !!m.encrypted_content,
          cachedHasContent: !!cached?.content,
        });
        if (!isIncoming) {
          ...
          // No cached plaintext found — fall through to shared ratchet decryption path
          // (needed for cross-device: new device has no cache but can decrypt with ratchet)
          console.log("[loadMessages] No cached plaintext for outgoing msg, will decrypt via ratchet");
        }

        let content = m.encrypted_content || "";
        const convKey = getConversationKey(m.sender_id, m.receiver_id);
        console.log("[loadMessages] Decrypting incoming message:", {
          id: m.id,
          convKey: convKey.slice(0, 20) + "...",
          hasRatchetState: !!(await getRatchetState(convKey)),
          hasEcdhKey: !!ecdhPrivateKey,
          hasOtherPublicKey: !!otherUser?.publicKey,
          encryptedContentStart: m.encrypted_content?.slice(0, 30),
        });
        ...
        if (!ratchetState) {
          try {
            const { initRatchetFromStorage } = await import("../utils/ratchetManager");
            console.log("[loadMessages] Ratchet state not in cache, loading from server...");
            await initRatchetFromStorage(ecdhPrivateKey);
            ratchetState = await getRatchetState(convKey);
            console.log("[loadMessages] Loaded ratchet state from server:", ratchetState ? "success" : "null");
          } catch (e) {
            console.warn("[loadMessages] Failed to load ratchet state from server:", e);
          }
        }

        // CRITICAL FIX: Ensure ratchet state has the original ECDH key for cross-device decryption
        if (ecdhPrivateKey && otherUser?.publicKey && typeof otherUser.publicKey === "string") {
          try {
            const { ensureRatchetStateWithOriginalKey } = await import("../utils/ratchetManager");
            ratchetState = await ensureRatchetStateWithOriginalKey(convKey, ecdhPrivateKey, otherUser.publicKey);
            console.log("[loadMessages] Ensured ratchet state has original ECDH key");
          } catch (e) {
            console.warn("[loadMessages] Failed to ensure original key:", e);
          }
        }

        if (!ratchetState && ecdhPrivateKey && otherUser?.publicKey) {
          console.log("[loadMessages] No ratchet state, creating fresh state from ECDH");
          const otherKey =
        ...
        console.log("[loadMessages] After ratchet setup:", {
        ...
        if (ratchetState && ecdhPrivateKey && m.encrypted_content) {
          ...
          try {
            if (m.encrypted_content.startsWith("{")) {
              ...
              console.log("[loadMessages] Parsed encrypted message:", {
                c_len: parsed.c?.length,
                n: parsed.n,
              });
              if (parsed.c && parsed.n !== undefined) {
                const dedupeKey = `${incomingKey}:${Number(parsed.n)}`;
                if (processedIncomingRef.current.has(dedupeKey)) {
                  console.log("[loadMessages] Skipping already processed message");
                  processed.push({ ...m, content: cached?.content || "" });
                  continue;
                }
                console.log("[loadMessages] Calling decryptWithRatchet for message:", m.id, "msgNum:", parsed.n, "useSendingChain:", !isIncoming);
                const dec = await decryptWithRatchet(
                  parsed.c,
                  convKey,
                  ecdhPrivateKey,
                  Number(parsed.n),
                  !isIncoming, // CRITICAL FIX: Use sending chain for outgoing messages
                );
                console.log("[loadMessages] Decrypt result:", dec ? `"${dec.slice(0, 50)}..."` : "null/empty");
                
                // CRITICAL FIX: Provide better error messaging
                if (dec) {
                  content = dec;
                  ...
                  // Decryption returned null - just show empty content
                  content = "";
                }
              } else {
                content = "";
      ...
      console.log("[loadMessages] Processed messages:", processed.map(m => ({
        id: m.id,
        content: m.content ? `"${m.content.slice(0, 30)}..."` : "(empty)",
      ...
      console.log("[loadMessages] Final merged messages:", merged.map(m => ({
        id: m.id,
        content: m.content ? `"${m.content.slice(0, 30)}..."` : "(empty)",
        sender: m.sender_id?.slice(0, 8),
      })));
      ...
      const masterKey = getMasterKeyFromStorage(ecdhPrivateKey);
      if (isValidMasterKey(masterKey)) {
        cacheSet(masterKey, convMsgCacheKey(user?.id, convId), merged).catch(() => {});
      }
  ...
  const loadReactions = async (convId) => {
    ...
    setReactions((prev) => {
      ...
      // Also check existing messages in prev that weren't in map (server has zero reactions)
      for (const msgId of Object.keys(prev)) {
        if (map[msgId]) continue; // already handled above — server has reactions for this
        // Server returned zero reactions for this message — clear everything
        delete merged[msgId];
      }
    ...
    const masterKey = getMasterKeyFromStorage(ecdhPrivateKey);
    if (isValidMasterKey(masterKey)) {
      cacheSet(masterKey, convReactCacheKey(user?.id, convId), map).catch(() => {});
    }
  };
  ...
  // Realtime reaction changes (added/removed) from other users
  useEffect(() => {
    if (!conversationId) return;
    const onReactionChanged = () => {
      loadReactionsRef.current?.(conversationId).catch(() => {});
    };
    window.addEventListener("zchat:reaction-changed", onReactionChanged);
    return () => {
      window.removeEventListener("zchat:reaction-changed", onReactionChanged);
    };
  }, [conversationId]);

  // Refetch when cache is cleared (from Settings "Clear cached messages")
  useEffect(() => {
    if (!conversationId || !user?.id) return;
    const onCacheCleared = (e) => {
      const prefix = e.detail?.prefix || "";
      if (prefix.includes(user.id)) {
        loadMessagesRef.current?.(conversationId, 60, false, true).catch(() => {});
        loadReactionsRef.current?.(conversationId).catch(() => {});
      }
    };
    window.addEventListener("chatapp-cache-cleared", onCacheCleared);
    return () => {
      window.removeEventListener("chatapp-cache-cleared", onCacheCleared);
    };
  }, [conversationId, user?.id]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setInput("");
      // Reset textarea height and re-focus (keep keyboard open on mobile)
  ...
  const handleDeleteMsgForAll = async (msgId) => {
    ...
    // Remove deleted messages from localStorage cache to prevent reappearance
    if (isValidMasterKey(masterKey)) {
      cacheClearByPrefix(convMsgCacheKey(user?.id, conversationId));
    }
    // Force refresh from server to ensure deleted messages are truly gone
    serverLoadedRef.current = false;
    loadMessagesRef.current?.(conversationId, 60, false, true).catch(() => {});
  };

  const handleStartEdit = (msg) => {
    console.log("[handleStartEdit] msg content:", msg?.content);
    closePopup();
    setEditingMsg(msg);
    // editInput will be synced via useEffect
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      try {
        ...
        if (!serverResponse) {
          console.warn("[Edit] Server returned empty response");
        }
  ...
  const doUnfriendFromChat = async () => {
    ...
    try {
      ...
      if (row?.id) await removeFriendship(row.id, otherUser.id);
  ...
  return (
    <div style={s.wrap}>
        ...
        {showScrollBtn && (
          <button
            ...
            style={{
              ...
              // Always position above the reply box using bottom
              bottom: selectMode
                ? 140
                : editingMsg
                  ? 140
          ...
          (editingMsg ? (
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <TextInput
                  value={editInput}
                  onChange={(text) => { setEditInput(text); }}
                  onKeyDown={(e) => {
      ...
      {popup && popupRect && (() => {
          ...
          // Position popup below the bubble if there's space, otherwise above.
          // Use the bubble's actual bounding rect — no height math needed.
          const popupOwnHeight = popup.type === "menu" ? MENU_H + GAP + REACT_H : REACT_H;
          const spaceBelow = INPUT_TOP - popupRect.bottom - GAP;
          const spaceAbove = popupRect.top - (contTop + HEADER_H) - GAP;

          let groupTop;
          if (spaceBelow >= popupOwnHeight) {
            // Fits below the bubble
            groupTop = popupRect.bottom + GAP;
          } else if (spaceAbove >= popupOwnHeight) {
            // Fits above the bubble
            groupTop = popupRect.top - GAP - popupOwnHeight;
          } else {
            // Clamp to available area
            groupTop = contTop + HEADER_H + 8;
          }

          const useFloatingLayer = false;
...
/**
 * ContentEditable input with per-line LTR/RTL direction.
 * Uses dir="auto" for native per-paragraph bidi.
 * Hidden mirror div for reliable height measurement.
 * JS-managed placeholder (no :empty tricks).
 */
...
function TextInput({
  ...
  const RTL_RE = /[\u0590-\u05FF\u0600-\u06FF\u0700-\u074F\u0750-\u077F\u08A0-\u08FF\uFB1D-\uFB4F\uFB50-\uFDFF\uFE70-\uFEFF]/;

  // Check if editor has visible text content
  const hasText = () => {
  ...
  const updatePlaceholder = () => {
    if (placeholderRef.current) {
      placeholderRef.current.style.display = hasText() ? 'none' : 'block';
    }
  };
  ...
  useEffect(() => {
    ...
    // Always clear when value is empty, even if focused (happens after send)
    if (!value) {
      el.innerHTML = '';
      el.style.height = minHeight + 'px';
      updatePlaceholder();
    } else {
      // Sync value to DOM - convert newlines to divs for multiline
  ...
  const handleInput = () => {
    if (!editorRef.current || !mirrorRef.current || composing.current) return;
    const el = editorRef.current;
    const mirror = mirrorRef.current;

    // Extract text preserving empty lines from block structure
    // Each <div>/<p> = one line. Join with \n.
    let text = '';
    ...
    // If no blocks at all, try innerText
    if (!text && el.children.length === 0) {
      text = el.innerText || '';
    }

    // Copy text to mirror for height measurement
    mirror.textContent = text;
    void mirror.offsetHeight; // force reflow
    const h = Math.max(minHeight, Math.min(mirror.scrollHeight, maxHeight));
  ...
  const handleKeyDown = (e) => {
    // Proactive direction switch: check the key BEFORE it's typed
    if (e.key.length === 1 && !e.ctrlKey && !e.metaKey) {
      if (RTL_RE.test(e.key)) {
  ...
  const handleBlur = (e) => {
    ...
    // Clean up stray <br> tags on blur if empty
    if (!hasText()) {
      e.target.innerHTML = '';
      updatePlaceholder();
    }
  ...
  useEffect(() => {
    ...
    mirror.textContent = '';
    el.style.height = minHeight + 'px';
    if (autoFocus) setTimeout(() => el.focus(), 50);
