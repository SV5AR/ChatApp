...
const Chat = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (initialOtherUser) {
        ...
        if (shouldLoadFromDB) {
          ...
          loadMessages(currentConvId, 60, false).then((fresh) => {
        ...
        // Load reactions in background (no UI block)
        if (!cachedReactions) {
          loadReactions(currentConvId).catch(() => {});
        }

        markMessagesAsSeen(currentConvId).catch(() => {});
        updateLastRead(currentConvId).catch(() => {});
    ...
    init().catch((err) => {
      if (!cancelled) {
        ...
        setStatus("Syncing chat in background.");
        setInitialising(false);
      }
    });
  ...
  useEffect(() => {
    ...
    cacheGet(rootKey, convReactCacheKey(user?.id, conversationId))
      .then((cached) => {
        if (cancelled || !cached || typeof cached !== "object") return;
        setReactions((prev) => (Object.keys(prev || {}).length ? prev : cached));
        _reactCache[conversationId] = cached;
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    console.log("[Chat] Setting up realtime for conv:", conversationId);
    ...
    const ch = supabase
      ...
      .on(
        ...
        async (p) => {
          console.log("[Chat] realtime INSERT received:", p);
          if (!p.new) return;
          ...
          // If the app-level delta already applied this change into the session cache,
          // prefer the authoritative session cache so UI updates consistently.
          try {
            const sessMsgs = _getConvMsgs(conversationId) || [];
            if (sessMsgs.some((m) => String(m.id) === String(p.new.id))) {
              setMessages(sessMsgs);
              return;
            }
          } catch (e) {
            // ignore
          ...
          // If this INSERT is our own message, try to reconcile optimistic local
          // message using client_message_id before doing a full reload.
          if (p.new.sender_id === user.id) {
              try {
                const convMsgs = _getConvMsgs(conversationId) || [];
                const clientId = p.new.client_message_id || p.new.clientMessageId || p.new.client_messageid || null;
                if (clientId) {
                  const found = convMsgs.find((m) => m.clientMessageId === clientId || m.client_message_id === clientId);
                  if (found) {
                  // Replace temp message id and update metadata atomically
                  const updated = convMsgs.map((m) =>
                    m.clientMessageId === clientId || m.client_message_id === clientId
                      ? { ...m, id: p.new.id, created_at: p.new.created_at || m.created_at, pending: false }
                      : m,
                  );
              ...
              // Fall back to reload on any error
              console.warn("[Chat-Realtime] reconciliation failed:", e);
            }

            // Fallback: reload recent messages to reconcile state (show sync UI)
            await loadMessages(conversationId, Math.max(60, messages.length + 12), true);
            return;
          }

          // Handle incoming message with Ratchet decryption (forward secrecy)
          let content = p.new.encrypted_content || "";
          const convKey = getConversationKey(p.new.sender_id, p.new.receiver_id);

          if (DBG) {
            console.debug("[Chat] realtime INSERT received", {
              messageId: p.new.id,
              senderId: p.new.sender_id,
              receiverId: p.new.receiver_id,
              convKey,
              hasEncryptedContent: !!p.new.encrypted_content,
              encryptedPreview: typeof p.new.encrypted_content === "string" ? p.new.encrypted_content.slice(0, 60) : null,
            });
          }
          
          // Try Ratchet decryption - first check cache, then reload from server if needed
          let ratchetState = await getRatchetState(convKey);
          
          // If no state in cache, try to reload all states from server
          if (!ratchetState) {
            try {
              ...
              console.warn("[Chat] Failed to reload ratchet states:", e);
            }
          }
          
          // If still no state, try to create from public key (for new conversations)
          ...
          if (DBG) {
            console.debug("[Chat] ratchet state for decrypt", {
              convKey,
              hasRatchetState: !!ratchetState,
              receivingMessageNumber: ratchetState?.receivingMessageNumber || null,
            });
          }
          
           if (ratchetState && ecdhPrivateKey && p.new.encrypted_content) {
            try {
              // Check if it's ratchet format (JSON with c and n)
              if (p.new.encrypted_content.startsWith("{")) {
                const parsed = JSON.parse(p.new.encrypted_content);
                if (parsed.c && parsed.n !== undefined) {
                  if (DBG) {
                    console.debug("[Chat] attempting decrypt", {
                      convKey,
                      messageNumberFromPayload: parsed.n,
                      ciphertextPreview: parsed.c?.slice(0, 40),
                    });
                  }
                  content = await decryptWithRatchet(
                    parsed.c,
                    convKey,
                    ecdhPrivateKey,
                    Number(parsed.n),
                  ...
                  if (DBG) {
                    console.debug("[Chat] decrypt result", {
                      convKey,
                      contentLength: content?.length || 0,
                      contentPreview: content?.slice(0, 30),
                    });
                  }
                   // decryption result logging removed
                } else {
                  // log removed
                }
              } else {
              ...
              console.warn("Ratchet decrypt failed on realtime:", e);
            }
          }

           // If decryption failed or not performed, schedule background decrypt
           if (!content || content === p.new.encrypted_content || content === "[Decrypt failed]") {
             // schedule background decrypt using bgQueue to avoid blocking realtime channel
             try {
               const bgQueueMod = await import("../utils/bgQueue");
                const bgQueue = bgQueueMod.default(5);
                 // background decrypt job log removed
                bgQueue.push(async () => {
                  try {
                   if (p.new.encrypted_content && ratchetState && ecdhPrivateKey) {
                     if (p.new.encrypted_content.startsWith("{")) {
                       const parsed = JSON.parse(p.new.encrypted_content);
                       if (parsed.c && parsed.n !== undefined) {
                         const dec = await decryptWithRatchet(parsed.c, convKey, ecdhPrivateKey, Number(parsed.n));
                         if (dec) {
                           const newMsg = { ...p.new, content: dec, reply_to_message_id: p.new.reply_to_message_id ?? null };
                           ...
                           if (!existing.some(m => m.id === newMsg.id)) {
                             ...
                             const rootKey = getRootKeyFromStorage(ecdhPrivateKey);
                             if (isValidRootKey(rootKey)) {
                               await import("../utils/cache").then(m => m.cacheSet(rootKey, convMsgCacheKey(user?.id, conversationId), next)).catch(() => {});
                             }
                           } else {
                             // replace edited/updated
                             const updated = existing.map(m => m.id === newMsg.id ? { ...m, content: dec } : m);
                             _setConvMsgs(conversationId, updated);
                             const rootKey = getRootKeyFromStorage(ecdhPrivateKey);
                             if (isValidRootKey(rootKey)) {
                               await import("../utils/cache").then(m => m.cacheSet(rootKey, convMsgCacheKey(user?.id, conversationId), updated)).catch(() => {});
                             }
                           }
                         }
                       }
                   ...
                   // ignore background decrypt errors
                 }
               }).catch(() => {});
             } catch (ee) {
               // ignore queue errors
             ...
             content = "[Decrypt failed]";
              // message decryption deferred to background (log removed)
           }

          const newMsg = { ...p.new, content };
          if (newMsg.reply_to_message_id === undefined) {
          ...
          setMessages((prev) => {
            ...
            const rootKey = getRootKeyFromStorage(ecdhPrivateKey);
            if (isValidRootKey(rootKey)) {
              cacheSet(rootKey, convMsgCacheKey(user?.id, conversationId), next).catch(() => {});
            }
            // message added to state (log removed)
            return next;
          });

          // If I'm the RECEIVER and I have the chat open → mark as seen
          if (p.new.receiver_id === user.id) {
      ...
      .on(
        ...
        async (p) => {
          ...
          setReactions((prev) => {
            ...
            // Replace temp reaction from same user+emoji, or skip if real ID already there
            const hasTmp = existing.some(
              (r) =>
            ...
            if (hasTmp) {
              const updated = {
                ...prev,
                [p.new.message_id]: existing.map((r) =>
                    r.id?.startsWith("tmp_") &&
                    r.user_id === p.new.user_id &&
                    r.emoji === emoji
                    ? normalized
      ...
      .on(
        ...
        async (p) => {
          ...
          setReactions(map);
          _reactCache[conversationId] = map;
          const rootKey = getRootKeyFromStorage(ecdhPrivateKey);
          if (isValidRootKey(rootKey)) {
              ...
              (async () => {
                ...
                if (ratchetState && ecdhPrivateKey) {
                  try {
                    ...
                    console.warn("Ratchet decrypt error in realtime:", e);
                  }
                }
      ...
      .on(
        ...
        (p) => {
          const senderId = p.old?.sender_id;
          const receiverId = p.old?.receiver_id;
          if (!senderId || !receiverId) return;
          const eventConvId = senderId === user.id ? receiverId : senderId;
          if (eventConvId !== conversationId) return;
          if (p.old?.id)
            setMessages((prev) => {
              const next = prev.filter((m) => m.id !== p.old.id);
                _setConvMsgs(conversationId, next);
              return next;
            });
      ...
      .subscribe((status, err) => {
        ...
        if (status !== "SUBSCRIBED") {
          console.log("[Chat] realtime FAILED:", status, err);
        }
      });
      ...
      try {
        ...
        if (change && change.eventType) {
          try {
            ...
            console.warn("[Chat] convUpdated delta apply failed, falling back to session cache", e);
          }
        }
        ...
        // Schedule background decrypt for any incoming messages that lack plaintext
        (async () => {
          try {
            const pendingToDecrypt = (sess || []).filter((m) => {
            ...
            for (const m of pendingToDecrypt) {
              bgQueue.push(async () => {
                try {
                  ...
                  if (ratchetState && ecdhPrivateKey && m.encrypted_content && m.encrypted_content.startsWith("{")) {
                    const parsed = JSON.parse(m.encrypted_content);
                    if (parsed.c && parsed.n !== undefined) {
                      const dec = await import("../utils/ratchetManager").then((mod) =>
                        mod.decryptWithRatchet(parsed.c, convKey, ecdhPrivateKey, Number(parsed.n)).catch(() => null),
                      );
                      if (dec) {
                        const existing = _getConvMsgs(conversationId) || [];
                        const updated = existing.map((mm) => (mm.id === m.id ? { ...mm, content: dec } : mm));
                        _setConvMsgs(conversationId, updated);
                        const rootKey = getRootKeyFromStorage(ecdhPrivateKey);
                        if (isValidRootKey(rootKey)) {
                          await import("../utils/cache").then((c) => c.cacheSet(rootKey, convMsgCacheKey(user?.id, conversationId), updated)).catch(() => {});
                        }
                        setMessages(updated);
                      }
                    }
  ...
  const loadMessages = async (convId, limit = 60, showSync = true) => {
    ...
    const run = async () => {
      ...
      try {
        const msgs = await getMessagesWithFriend(convId, limit);
      ...
      const cachedByClientId = new Map(
        ((_getConvMsgs(convId) || []).filter(Boolean))
          ...
          .map((m) => [String(m.client_message_id || m.clientMessageId).trim(), m]),
      );
      ...
      for (const m of msgs) {
        ...
        const isIncoming = m.receiver_id === user?.id;
        if (!isIncoming) {
          const cacheByClient = cachedByClientId.get(
            String(m.client_message_id || m.clientMessageId || "").trim(),
          );
        ...
        if (DBG) {
          console.debug("[Chat] loadMessages decrypt attempt", {
            messageId: m.id,
            isIncoming,
            convKey,
        ...
        if (DBG) {
          console.debug("[Chat] loadMessages ratchet state", {
            convKey,
            hasRatchetState: !!ratchetState,
            receivingMessageNumber: ratchetState?.receivingMessageNumber || null,
          });
        ...
        if (ratchetState && ecdhPrivateKey && m.encrypted_content) {
          try {
            if (m.encrypted_content.startsWith("{")) {
              ...
              if (parsed.c && parsed.n !== undefined) {
                if (DBG) {
                  console.debug("[Chat] loadMessages attempting decrypt", {
                    messageId: m.id,
                    messageNumber: parsed.n,
                    ciphertextPreview: parsed.c?.slice(0, 40),
                  });
                }
            ...
            console.warn("Ratchet decrypt error:", e);
            content = "[Decrypt failed]";
          }
      ...
      const merged = [
        ...processed,
        ...pendingLocal.filter(
          (p) => !processed.some((m) => String(m.id) === String(p.id)),
        ),
      ].sort(
        (a, b) =>
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        await loadMessagesRef.current?.(conversationId, lim, false);
    ...
    const everyMs = rtSubscribed ? 6500 : 2200;
    const boot = window.setTimeout(poll, rtSubscribed ? 3200 : 900);
  ...
  const flushOutbox = useCallback(async () => {
    ...
    try {
      ...
      loadMessagesRef.current?.(conversationId, Math.max(60, messagesLenRef.current + 10), false).catch(() => {});
    } finally {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      let error = null;
      try {
        await editMessageById(editingMsg.id, encryptedContent);
      } catch (e) {
        error = e;
      }
      ...
      if (error) {
        ...
        setStatus("Failed to edit message: " + error.message);
        return;
      }
  ...
  const handleReact = async (messageId, emoji) => {
    ...
    if (existing) {
      // If this is an optimistic temp reaction, cancel locally only.
      if (existing.id?.startsWith("tmp_")) {
        setReactions((prev) => {
          const updated = {
            ...prev,
      ...
      reactionQueueRef.current.push({
        ...
        emoji,
        friendId: otherUser?.id,
        friendPublicKey: otherUser?.publicKey,
      });
      flushReactionQueue();
    }
  };
  ...
  const flushReactionQueue = useCallback(async () => {
    ...
    try {
      while (reactionQueueRef.current.length > 0) {
        ...
        if (!item) continue;
        if (item.type === "clear") {
          await clearOwnReaction(item.messageId, item.reactionId);
        } else if (item.type === "upsert") {
          await upsertReaction(
            item.messageId,
            item.emoji,
            item.friendId,
            item.friendPublicKey,
          );
        }
      }
    } catch {
      // best effort; UI optimistic updates + polling/realtime will reconcile
    } finally {
  ...
  const s = {
    wrap: {
      ...
      background: embedded ? "transparent" : theme.bg,
    ...
    <div style={s.wrap}>
            ...
            {messages.length === 0 ? (
              ...
              {(() => {
                ...
                return items.map((item) => {
                  ...
                  if (DBG && msg && (!msg.content || msg.content === "[Decrypt failed]" || msg.content === "[Encrypted message]")) {
                    try {
                      console.debug("[Chat] message missing/plaintext", {
                        id: msg.id,
                        hasEncrypted: typeof msg.encrypted_content === "string" && msg.encrypted_content.trim().length > 0,
                        encryptedPreview: typeof msg.encrypted_content === "string" ? msg.encrypted_content.slice(0, 40) : null,
                        content: msg.content,
                        clientMessageId: msg.clientMessageId || msg.client_message_id || null,
                        pending: msg.pending,
      ...
      {popup &&
        ...
        (() => {
          ...
          const popupWidth = Math.min(popupRect.width, Math.max(140, contWidth - 16));
          const leftCandidate = isMine
            ? contLeft + contWidth - popupWidth - 8
            : contLeft + 8;
          const leftPos = Math.max(
            contLeft + 8,
            Math.min(leftCandidate, contLeft + contWidth - popupWidth - 8),
          );
                          ...
                          >
                            {customReactions.map((e) => {
                              ...
                              return (
                                <button
                                  key={e}
                                  onClick={() => handleReact(popup.msgId, e)}
                                  style={{
                                    background: active
                                      ? `${theme.primary}30`
                                      : "none",
            ...
            >
              {[
                ...
                <button
                  key={e}
                  onClick={() => {
                    handleReact(showAddEmoji.msgId, e);
