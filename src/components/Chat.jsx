...
const Chat = ({
    ...
    const ch = supabase
      ...
      .on(
        ...
        (p) => {
          ...
          // Mark as seen in background (non-blocking)
          const peerId = p.new.sender_id || otherUser?.id || conversationId;
          markMessagesAsSeen(peerId).catch(() => {});
          updateLastRead(peerId).catch(() => {});
          // Force refresh messages from DB so ticks update immediately
      ...
      .on(
        ...
        (p) => {
          if (!p.new?.id) return;
          // Only care about read_at updates for outgoing messages
  ...
  const loadMessages = async (convId, limit = 60, showSync = true, bypassCache = false) => {
    ...
    const run = async () => {
      ...
      for (const msg of existing) {
        ...
        if (serverMsg) {
          // Merge: keep local content if server has none, prefer server metadata
          merged.push({
            ...serverMsg,
          ...
          // Message not on server yet (still pending) — keep as-is
          merged.push(msg);
          seenIds.add(String(msg.id));
        }
      }
  ...
  // Mark messages as read immediately when they appear on screen
  useEffect(() => {
    if (!conversationId || !user?.id || !otherUser?.id) return;
    const hasUnseenIncoming = messages.some(
      (m) => m.sender_id === otherUser.id && !m.read_at && !m.seen_at,
    );
  ...
  const handleReact = async (messageId, emoji) => {
    ...
    if (existing) {
      // Remove reaction immediately (optimistic)
      setReactions((prev) => {
        const updated = {
          ...prev,
      ...
      clearOwnReaction(messageId, existing.id).catch(() => {});
    } else {
      // Add reaction immediately (optimistic)
