...
const Friends = ({
  ...
  const measureTabs = useCallback(() => {
    if (tabTrackRef.current) {
      ...
      setIsCompactMode(trackWidth < 380);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (tabTrackRef.current) observer.observe(tabTrackRef.current);
    return () => {
      clearTimeout(timer);
      observer.disconnect();
    };
  ...
  useEffect(() => {
    ...
    if (activeTab === "received" && received.length > 0) {
      const oldestReceived = received.reduce((min, r) => 
        new Date(r.created_at) < new Date(min.created_at) ? r : min, received[0]);
      if (new Date(oldestReceived.created_at) > new Date(newLastSeen.received)) {
        newLastSeen.received = now;
        updated = true;
      }
    }
    
    if (activeTab === "sent" && sent.length > 0) {
      const oldestSent = sent.reduce((min, r) => 
        new Date(r.created_at) < new Date(min.created_at) ? r : min, sent[0]);
      if (new Date(oldestSent.created_at) > new Date(newLastSeen.sent)) {
        newLastSeen.sent = now;
        updated = true;
      }
    }
    ...
    if (updated) {
      setLastSeen(newLastSeen);
      try {
        localStorage.setItem("friend_tabs_last_seen", JSON.stringify(newLastSeen));
      } catch {}
    }
  }, [activeTab, received.length, sent.length]);
  ...
  const doUnfriend = async () => {
    ...
    try {
      ...
      if (fr?.id) {
        await removeFriendship(fr.id, friendId);
      }
      await fetchAll(uid);
    ...
    >
      ...
      >
        ...
        <div
          ...
          flex: 1,
          overflowY: "auto",
          padding: embedded ? "8px 14px 14px" : "8px 20px 20px",
        }}
