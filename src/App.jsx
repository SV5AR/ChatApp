...
import { clearSigningKeys, clearRememberedMasterKey, loadMasterKey, hasRememberedMasterKey } from "./lib/edgeApi";
...
const _addMessageToCache = (convId, message) => {
  ...
  // CRITICAL FIX: Also sync to window._sessionCache
  ...
  if (!window._sessionCache.convMessages[convId].find((m) => m.id === message.id)) {
    window._sessionCache.convMessages[convId].push(message);
  }
};
...
const _setMessagesInCache = (convId, messages) => {
  ...
  // CRITICAL FIX: Also sync to window._sessionCache so Chat.jsx can access preloaded messages
  if (!window._sessionCache) window._sessionCache = { convMessages: {}, convReactions: {} };
  if (!window._sessionCache.convMessages) window._sessionCache.convMessages = {};
  window._sessionCache.convMessages[convId] = messages;
};
...
const trySetAuthUserContext = async (userId) => {
  if (!userId) return;
  // No session token needed — Ed25519 signing handles auth.
};

const App = () => {
  const { theme, layoutName } = useTheme();
  useEffect(() => {
    // Mark that the next apply() run for sidebar layout should perform the
    // initial default expansion logic. This flag is cleared by apply() itself
    // so it only affects the first measurement after switching to the sidebar layout.
    if (layoutName === "sidebar") initialSidebarApplyRef.current = true;
  }, [layoutName]);
  ...
  const initialSidebarApplyRef = useRef(true);
  const [collapsedReservePx, setCollapsedReservePx] = useState(() => {
    // initialize collapsed reserve based on initial viewport
    try {
      return window.innerWidth <= 760 ? Math.round(window.innerWidth * 0.10) : 86;
    } catch {
      return 86;
    }
  });
  const [contentAreaRect, setContentAreaRect] = useState({ left: 0, width: 0 });
  ...
  // ── Auth (signature-based, no session tokens) ──────────────────────────────
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      if (lockActive && lockedUserId) {
        // Verify Remember Me key exists so we can actually unlock
        ...
        if (hasRememberMe) {
          ...
          // Lock state set but no Remember Me key → stale state from logout, clear it
          setAppLockState(false);
        }
      }

      // Check if signing keys exist in sessionStorage (from active session or Remember Me)
      const signingSecretKey = sessionStorage.getItem("userEd25519SecretKey");
      const userId = sessionStorage.getItem("userId");

      if (signingSecretKey && userId) {
        // Keys already in sessionStorage — active session
        try {
        ...
        // No keys in sessionStorage — try to restore from Remember Me (localStorage)
        try {
          ...
          if (remembered && remembered.masterKeyHex) {
            ...
            setInitStage(0.08, "Restored remembered session...");
            try {
              await trySetAuthUserContext(identity.userId);
              await initUser({ id: identity.userId });
            } catch (e) {
              console.error("Remembered session restore failed:", e);
              if (isBackendObjectMissingError(e)) {
                setBackendSyncIssue(backendSetupMessage);
              }
              wipeRuntimeSessionForLock();
            ...
            // No remembered key either — show auth page
            setInitStage(1, "Ready");
            setAppLoading(false);
          }
        } catch (e) {
  ...
  const initUser = async (authUser) => {
    ...
    try {
      // CRITICAL FIX: Initialize ratchet state from server BEFORE loading messages
      // This ensures cross-device compatibility - we load the latest ratchet state
      // so we can decrypt messages that were encrypted on other devices
      if (savedMk) {
        try {
          const { initRatchetFromStorage } = await import("./utils/ratchetManager");
          await initRatchetFromStorage(savedMk);
          console.log("[initUser] Ratchet states loaded from server for cross-device sync");
        } catch (e) {
          console.warn("[initUser] Failed to load ratchet states:", e);
        }
      }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (p) => {
          markSyncing(true);
          // Broadcast to all open chats — Chat.jsx reconciles with DB
          window.dispatchEvent(new CustomEvent("zchat:reaction-changed", {
            detail: { type: p.eventType, reaction: p.new || p.old },
          }));
          scheduleConversationRefresh(120);
        },
      )
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      // Only set the default expanded/collapsed state on the first run after
      // the effect mounts (or when layoutName changes). This avoids racing
      // with explicit user interactions (tapping to expand on small screens).
      if (initialSidebarApplyRef.current) {
        if (!compact) setSidebarExpanded(true);
        else if (!sidebarSearchOpen) setSidebarExpanded(false);
        initialSidebarApplyRef.current = false;
      }

      // Compute the collapsed reserve locally so we can use it immediately
      // when measuring the content area (avoid relying on setState being flushed).
      const localCollapsedReserve = layoutName !== "sidebar" ? 86 : !compact ? 86 : Math.round(window.innerWidth * 0.10);
      if (collapsedReservePx !== localCollapsedReserve) setCollapsedReservePx(localCollapsedReserve);

      // Update content area rect so the fixed overlay is centered relative to the
      // visible right-side area (the section-fade element). Use requestAnimationFrame
      // so DOM layout is stable. When the sidebar is acting as an overlay we should
      // measure excluding the *current* sidebar width (not the collapsed reserve).
      requestAnimationFrame(() => {
        try {
          const el = sectionFadeRef.current;

          // Prefer measuring the actual sidebar DOM width when available so
          // percentage widths (relative to the app container) are accurate.
          let measuredSidebarWidth = Number(sidebarWidthPx || 0);
          try {
            const sb = sidebarRootRef.current;
            if (sb && typeof sb.getBoundingClientRect === "function") {
              const sbRect = sb.getBoundingClientRect();
              if (sbRect && sbRect.width) measuredSidebarWidth = Math.round(sbRect.width);
            }
          } catch (e) {
            // ignore measurement errors and fall back to sidebarWidthPx
          }

          const localOverlayActive = layoutName === "sidebar" && measuredSidebarWidth > Number(localCollapsedReserve || 0);
          const usedReserve = localOverlayActive ? measuredSidebarWidth : Number(localCollapsedReserve || 0);

          if (el && typeof el.getBoundingClientRect === "function") {
            const r = el.getBoundingClientRect();
            const left = Math.round(r.left + usedReserve);
            const width = Math.round(Math.max(0, r.width - usedReserve));
            setContentAreaRect({ left, width });
          } else {
            setContentAreaRect({ left: usedReserve, width: Math.max(0, window.innerWidth - usedReserve) });
          }
        } catch (e) {
          const fallbackReserve = Number(sidebarWidthPx || 0) > 0 ? Number(sidebarWidthPx || 0) : collapsedReservePx;
          setContentAreaRect({ left: fallbackReserve, width: Math.max(0, window.innerWidth - fallbackReserve) });
        }
      });
    };
    ...
    // Also observe DOM size changes for the sidebar and the content container
    // so we update the overlay position when CSS transitions/animations change
    // their layout (ResizeObserver captures width changes during transitions).
    let ro = null;
    try {
      if (typeof window !== "undefined" && typeof window.ResizeObserver === "function") {
        ro = new window.ResizeObserver(() => {
          // run the same apply logic to recompute measurements
          apply();
        });
        if (sidebarRootRef.current) ro.observe(sidebarRootRef.current);
        if (sectionFadeRef.current) ro.observe(sectionFadeRef.current);
      }
    } catch (e) {
  ...
  }, [layoutName, sidebarSearchOpen, sidebarCompact, sidebarExpanded, sidebarFriendsFocus]);
  ...
  useEffect(() => {
    // Close the sidebar when clicking outside while it's expanded (overlay mode).
    if (layoutName !== "sidebar" || !sidebarExpanded) return;
  ...
  }, [layoutName, sidebarExpanded, sidebarSearchOpen]);
  ...
  const handleUnfriend = async (convId, otherUserId) => {
    ...
    try {
      ...
      if (friendship?.id) {
        await removeFriendship(friendship.id, friendId);
      }
    } catch (e) {
  ...
  // Determine whether the sidebar is currently acting as an overlay that
  // covers more than the reserved collapsed width. When true, we should show
  // the backdrop and dim the main content.
  const sidebarOverlayActive =
    layoutName === "sidebar" && Number(sidebarWidthPx || 0) > Number(collapsedReservePx || 0);
  ...
  return (
    ...
    >
        ...
        <div
            ...
            onClick={() => {
              if (!sidebarIsExpanded) setSidebarExpanded(true);
            }}
            style={{
              width: sidebarWidth,
              // Always render the sidebar as an overlay so it does not shift main content
              position: "absolute",
              left: 0,
              top: 0,
              bottom: 0,
              // When overlayed, ensure it sits above main content
              zIndex: sidebarExpanded ? 200 : 10,
              ...
              padding: sidebarCompact
                ? sidebarIsExpanded
                  ...
                  : "18px 2px 12px"
                : "20px 12px 16px",
              transition: `width ${motionSlow} ${easeEmphasis}, opacity ${motionNormal} ${easeStandard}, transform ${motionSlow} ${easeEmphasis}`,
          ...
          <div
            ...
            style={{
              ...
              paddingBottom: 0,
              overflow: "hidden",
              // Reserve space on the right (or left) so overlayed collapsed sidebar
              // does not visually overlap content when unfocused. We apply padding
              // equal to the collapsedReservePx on the side where the sidebar sits.
              paddingLeft: layoutName === "sidebar" ? `${collapsedReservePx}px` : 0,
              transition: `padding-left ${motionSlow} ${easeEmphasis}`,
            }}
          >
          {/* Overlay backdrop when sidebar is expanded in compact mode */}
          {layoutName === "sidebar" && sidebarOverlayActive && (
            <div
              style={{
                position: "absolute",
                inset: 0,
                ...
                zIndex: 150,
                transition: `opacity ${motionNormal} ${easeStandard}`,
              }}
              onClick={() => {
                // Clicking on the backdrop should close the expanded overlay sidebar
                setSidebarExpanded(false);
                setSidebarSearchOpen(false);
              }}
            />
          )}
          {/* Magnifier + hint overlay sitting inside the available right-side area
              (outside the sidebar) and above the backdrop. Render via portal
              into document.body to avoid stacking-context/transform issues. */}
          {layoutName === "sidebar" && sidebarSearchOpen && typeof document !== "undefined"
            ? createPortal(
                <div
                  style={{
                    position: "fixed",
                    top: 0,
                    left: `${contentAreaRect.left}px`,
                    width: `${contentAreaRect.width}px`,
                    bottom: 0,
                    zIndex: 1000,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    pointerEvents: "none",
                    animation: "msgAppear 0.25s cubic-bezier(0.22,1,0.36,1) both",
                  }}
                >
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 18, pointerEvents: "none" }}>
                    <div
                    ...
                    <div style={{ textAlign: "center", pointerEvents: "none" }}>
                      <div style={{ color: theme.text3, fontSize: 14, fontWeight: 600, letterSpacing: 0.15, marginBottom: 4 }}>Searching...</div>
                      <div style={{ color: theme.text3, fontSize: 11, opacity: 0.6 }}>Type a name or paste a user ID</div>
                    </div>
                  </div>
                </div>,
                document.body,
          ...
          <div
            style={{
              ...
              transition: `opacity ${motionNormal} ${easeStandard}`,
              opacity:
                (layoutName === "sidebar" && (sidebarOverlayActive || sidebarFriendsFocus)) || (layoutName === "modal" && selectedConversation)
                  ? 0.3
                  : 1,
              pointerEvents:
                (layoutName === "sidebar" && (sidebarOverlayActive || sidebarFriendsFocus)) || (layoutName === "modal" && selectedConversation)
          ...
          {layoutName === "sidebar" ? (
            <div style={{ position: "relative", flex: 1, display: "flex", flexDirection: "column", minHeight: 0, minWidth: 0 }}>
              {/* Search overlay that covers the right side when search is active.
                  The magnifier + hint are rendered in the fixed overlay above the backdrop. */}
              {sidebarSearchOpen && (showSettings || showFriends || selectedConversation) && (
                <div
                  style={{
                    position: "absolute",
                    top: 0,
                ...
                >
                  {/* magnifier + hint intentionally moved to the top-level fixed overlay */}
                </div>
              )}
              {showSettings ? (
                ...
                layoutName === "sidebar" && sidebarSearchOpen ? null : (
                <div
                  style={{
                    flex: 1,
                    display: "flex",
                ...
                <div style={{ textAlign: "center" }}>
                  ...
                  {layoutName === "sidebar" && (
                    ...
                    >
                      <span>or tap the <span style={{ display: "inline-flex", verticalAlign: "middle", margin: "0 2px" }}><SearchFriendIcon size={14} style={{ color: theme.primary }} /></span> icon to add a friend by ID</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div
                style={{
                  ...
                  position: "relative",
                  overflow: "hidden",
                  pointerEvents: !rightPanelReady ? "none" : "auto",
                }}
              >
                {/* Unified backdrop above will handle closing overlay sidebar; remove compact-only duplicate overlay */}
                <Chat
              ...
              {telegramTab === "contacts" ? (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
                  ...
                  <div style={{
                    ...
                    paddingLeft: 14,
                    paddingRight: 14,
                    paddingBottom: TELEGRAM_NAV_BOTTOM_PAD
                  }}>
                  <div
                    style={{
                      position: "relative",
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
                ...
                <div style={{ 
                  ...
                  <Settings
                    ...
                    onBlockedListClick={() => setShowBlockedList(true)}
                  />
                </div>
              </div>
            ) : null}
        ...
        {showBlockedList && (
          <BlockedListModal onClose={() => setShowBlockedList(false)} />
        )}
            ...
            return (
              <div
                style={{
                  ...
                  inset: 0,
                  zIndex: 120,
                  opacity: layoutName === "sidebar" && sidebarOverlayActive ? 0 : 1,
                  transform: layoutName === "sidebar" && sidebarOverlayActive ? "translateX(20px) scale(0.985)" : "translateX(0) scale(1)",
                  transition:
                    "opacity 0.22s ease, transform 0.24s cubic-bezier(0.22,1,0.36,1)",
                   pointerEvents: layoutName === "sidebar" && sidebarOverlayActive ? "none" : "auto",
                  background: theme.surface,
                }}
              >
                <Chat
