...
export function getMasterKeyFromStorage() {
...
export async function saveToCache(keyType, uid, data, extraKey = null) {
  const masterKey = getMasterKeyFromStorage();
  if (!isValidMasterKey(masterKey)) return;
  
  const key = getCacheKey(keyType, uid, extraKey);
  if (key) {
    await cacheSet(masterKey, key, data);
  }
}
...
export async function loadFromCache(keyType, uid, extraKey = null) {
  ...
  const key = getCacheKey(keyType, uid, extraKey);
  if (key) {
    return await cacheGet(masterKey, key);
  }
  return null;
}
