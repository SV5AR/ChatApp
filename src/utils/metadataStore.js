...
export async function loadFriendTabsLastSeen() {
  ...
  try {
    const masterKey = sessionStorage.getItem("userPrivateKey") || "";
    if (/^[0-9a-f]{64}$/i.test(masterKey) && userId) {
      const v = await cacheGet(masterKey, `meta_lastseen_${userId}`);
...
export async function saveFriendTabsLastSeen(value) {
  ...
  try {
    const masterKey = sessionStorage.getItem("userPrivateKey") || "";
    if (/^[0-9a-f]{64}$/i.test(masterKey) && userId) {
      await cacheSet(masterKey, `meta_lastseen_${userId}`, merged);
      return;
    }
  } catch (e) {
    // ignore
...
export async function loadCustomReactions(defaultReactions) {
  ...
  try {
    const masterKey = sessionStorage.getItem("userPrivateKey") || "";
    if (/^[0-9a-f]{64}$/i.test(masterKey) && userId) {
      const v = await cacheGet(masterKey, `meta_custom_reacts_${userId}`);
...
export async function saveCustomReactions(reactions) {
  ...
  try {
    const masterKey = sessionStorage.getItem("userPrivateKey") || "";
    if (/^[0-9a-f]{64}$/i.test(masterKey) && userId) {
      await cacheSet(masterKey, `meta_custom_reacts_${userId}`, reactions);
      return;
    }
  } catch (e) {
    // ignore
