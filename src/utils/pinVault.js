...
export function getPinStrengthError(pin) {
  const value = String(pin || "");
  if (!isValidPin(value)) {
    return `Password must be ${PIN_MIN_LENGTH}-${PIN_MAX_LENGTH} characters`;
  }
  if (COMMON_WEAK_PINS.has(value.toLowerCase())) {
...
export async function encryptMasterKeyWithPin(masterKey, pin) {
  if (!isValidPin(pin)) {
  ...
  const payload = new TextEncoder().encode(
    JSON.stringify({ masterKey: masterKey.trim().toLowerCase(), v: 1, ts: Date.now() }),
  );
...
export async function decryptMasterKeyWithPin(vault, pin) {
  ...
  return String(parsed?.masterKey || parsed?.phrase || "").trim().toLowerCase();
}

export const encryptPhraseWithPin = encryptMasterKeyWithPin;
export const decryptPhraseWithPin = decryptMasterKeyWithPin;
