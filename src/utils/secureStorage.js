 ...
 * Uses in-memory cache + persistent localStorage so saved login/password vault survive refresh.
 * NOTE: The phrase is NEVER stored — only the encrypted master key + chain code.
 */
...
const MASTER_KEY_PREFIX = "master_key_";
 ...
 * Preserves persistent "Remember Me" data (password vault, root keys, accounts)
    ...
    Object.keys(localStorage)
      .filter(
        (k) =>
          ...
          k.startsWith("master_key_") ||
          k.startsWith("zchat_master_key"),
      )
 ...
 * Does NOT touch keys, password vaults, or other accounts.
 */
...
export async function savePinVaultForUser(userId, pinVault, username = null) {
  ...
  if (!uid) throw new Error("Missing user id for password vault save");
...
export async function removeAccount(userId) {
  ...
  await clearMasterKeyForUser(uid);
  deleteValue(`account_${uid}`);
}

export async function saveMasterKeyForUser(userId, privateKeyHex) {
  const uid = normalizeVaultUserId(userId);
  ...
  setValue(`${MASTER_KEY_PREFIX}${uid}`, {
    userId: uid,
    mode: "plain",
    privateKey: key,
    timestamp: Date.now(),
...
export async function markMasterKeyEncryptedForUser(userId) {
  const uid = normalizeVaultUserId(userId);
...
export async function loadMasterKeyForUser(userId) {
  const uid = normalizeVaultUserId(userId);
  if (!uid) return null;
  const row = getValue(`${MASTER_KEY_PREFIX}${uid}`);
  if (String(row?.mode || "") === "password_vault") return null;
  const key = String(row?.privateKey || "").trim().toLowerCase();
  if (!/^[0-9a-f]{64}$/i.test(key)) return null;
  return key;
}

export async function clearMasterKeyForUser(userId) {
  const uid = normalizeVaultUserId(userId);
  if (!uid) return;
  deleteValue(`${MASTER_KEY_PREFIX}${uid}`);
}
