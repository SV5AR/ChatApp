...
const MK_OBFUSCATED_KEY = "zchat_master_key_obf";
...
/**
 * Simple XOR obfuscation - fast, no user action needed.
 * Not secure encryption, but prevents casual reading of key from localStorage.
 */
...
function xorDeobfuscate(obfuscated, key) {
  const keyBytes = new TextEncoder().encode(key);
  const result = new Uint8Array(obfuscated.length);
  for (let i = 0; i < obfuscated.length; i++) {
    result[i] = obfuscated[i] ^ keyBytes[i % keyBytes.length];
  }
  return result;
}

/**
 * Persist the master key with obfuscation (no PIN) or encryption (with PIN).
 * If a password is provided → encrypt with PBKDF2-derived key (secure).
 * If no password → XOR obfuscate with random salt (not plaintext, no user action needed).
 */
export async function persistMasterKey(masterKeyHex, password = "") {
  console.log("[persistMasterKey] masterKeyHex:", masterKeyHex?.slice(0, 16), "password:", password);
  try {
    if (password && password.trim().length > 0) {
      const { encryptMasterKeyWithPin } = await import("../utils/pinVault");
      const vault = await encryptMasterKeyWithPin(masterKeyHex, password);
      localStorage.setItem(MK_ENCRYPTED_KEY, JSON.stringify(vault));
      localStorage.removeItem(MK_OBFUSCATED_KEY);
    } else {
      const salt = crypto.getRandomValues(new Uint8Array(32));
      const saltB64 = toB64(salt);
      const obfuscated = xorObfuscate(masterKeyHex, saltB64);
      const vault = {
        salt: saltB64,
        data: toB64(obfuscated),
      };
      localStorage.setItem(MK_OBFUSCATED_KEY, JSON.stringify(vault));
 ...
 * Returns { masterKeyHex } for obfuscated mode, or null if not available.
 * If password-encrypted vault exists, returns { encrypted: true }.
 */
export async function loadMasterKey() {
  try {
    const obfuscatedRaw = localStorage.getItem(MK_OBFUSCATED_KEY);
    if (obfuscatedRaw) {
      try {
        const vault = JSON.parse(obfuscatedRaw);
        if (!vault.salt || !vault.data) {
          return null;
        }
        const obfuscated = fromB64(vault.data);
        const masterKeyHex = new TextDecoder().decode(xorDeobfuscate(obfuscated, vault.salt));
...
/**
 * Check if a remembered master key exists (either obfuscated or encrypted).
 */
export function hasRememberedMasterKey() {
  try {
    if (localStorage.getItem(MK_OBFUSCATED_KEY)) return true;
