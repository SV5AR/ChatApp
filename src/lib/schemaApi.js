...
export async function getProfile(userId) {
  ...
  try {
    ...
    if (String(err?.message || "").toLowerCase().includes("unauthorized session")) {
      // Session is invalid/expired — treat as missing profile for UI resilience
      return null;
    }
    throw err;
  }
...
export async function getFriendships(status) {
  try {
    ...
    if (String(err?.message || "").toLowerCase().includes("unauthorized session")) {
      // Return empty list when session is unauthorized; UI will prompt re-auth if needed
      return [];
    }
    throw err;
  }
...
export async function getMessagesWithFriend(friendId, limit = 200) {
  const rootPrivateKeyHex = await getRootPrivateKey();
  const noise = rootPrivateKeyHex ? generateRequestNoise() : null;
...
export async function blockUser(blockerId, blockedId) {
  ...
  await queuedMutationWithFallback("block_user", payload, () => 
...
export async function unblockUser(blockerId, blockedId) {
  ...
  if (!blocker || !blocked) return false;
  const payload = { blockedId: blocked, nonce: randomNonceHex() };
  await queuedMutationWithFallback("unblock_user", payload, () => 
