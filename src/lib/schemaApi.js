...
import { cacheClearByPrefix } from "../utils/cache";
  ...
  return async (...args) => {
    ...
    if (masterKeyHex) {
      for (const field of decryptFields) {
        if (result?.[field]) {
          result[field] = await decryptPayload(result[field], masterKeyHex);
        }
      }
    }
...
async function encryptOutgoing(masterKeyHex, data, fields) {
  if (!masterKeyHex || !data) return data;

  const encrypted = { ...data };
  for (const field of fields) {
    if (encrypted[field]) {
      encrypted[field] = await encryptPayload(encrypted[field], masterKeyHex);
    }
  }
  return encrypted;
}
...
export async function sendFriendRequest(receiverId, encryptedKeyBundle = null) {
  const masterKeyHex = await getMasterKey();
  const noise = masterKeyHex ? generateRequestNoise() : null;
...
export async function removeFriendship(friendshipId, friendId) {
  const uid = normalizeUserId(sessionStorage.getItem("userId"));
  const fid = normalizeUserId(friendId);
  ...
  return queuedMutationWithFallback("remove_friendship", payload, () => 
    ...
    cacheInvalidate(/^chats_/);
    cacheInvalidate(/^chatRows_/);
    // Clear localStorage message caches for this friendship
    if (uid && fid) {
      cacheClearByPrefix(`conv_msgs_${uid}_${fid}`);
      cacheClearByPrefix(`conv_reactions_${uid}_${fid}`);
    }
    return result;
  });
}
...
export async function editMessageById(messageId, encryptedContent) {
  ...
  try {
    const body = await edgePost("/messages/edit", payload);
...
export async function blockUser(blockerId, blockedId) {
  ...
  return queuedMutationWithFallback("block_user", payload, () => 
    ...
    // Clear localStorage message caches when blocking
    if (uid && blocked) {
      cacheClearByPrefix(`conv_msgs_${uid}_${blocked}`);
      cacheClearByPrefix(`conv_reactions_${uid}_${blocked}`);
    }
    return result;
  });
}
...
export async function hideChatForMe(friendId) {
  ...
  return edgePost("/messages/hide", {
    ...
    // Clear localStorage message caches for this chat
    if (uid && fid) {
      cacheClearByPrefix(`conv_msgs_${uid}_${fid}`);
      cacheClearByPrefix(`conv_reactions_${uid}_${fid}`);
    }
...
export async function deleteChatForEveryone(friendId) {
  const uid = normalizeUserId(sessionStorage.getItem("userId"));
  const convId = normalizeUserId(friendId);
  ...
  return edgePost("/chat/delete", {
    ...
    // Clear localStorage message cache for this conversation
    if (uid && convId) {
      cacheClearByPrefix(`conv_msgs_${uid}_${convId}`);
      cacheClearByPrefix(`conv_reactions_${uid}_${convId}`);
    }
...
export async function updateEncryptedStatus(isOnline) {
  ...
  const encryptedStatus = await encryptStatus(isOnline, masterKeyHex);
...
export async function sendEncryptedTypingIndicator(recipientId, isTyping) {
  ...
  const encrypted = await encryptTypingIndicator(recipientId, isTyping, masterKeyHex);
  const noise = generateRequestNoise();
